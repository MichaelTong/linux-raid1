		...
		bool is_async)
{
    ...
    {
        printk("Time: ");
        for(i = 0; i < dio->bios_cnt; i++)
        {
            printk("%d: %d, ", i, ktime_to_ns(ktime_sub(dio->bios[i]->e1, dio->bios[i]->b1)));
        }
        printk("\n");
    }
	...
	dio_submit_t submit_io,	int flags)
{
    ...
    dio->bios = (struct bio**)kmalloc(4*sizeof(struct bio*), GFP_KERNEL);
    dio->bios_cnt = 0;
    //printk("MikeT: %s %s %d, raid:%s\n", __FILE__, __func__, __LINE__, dio->isRAID?"true":"false");
	dio->flags = flags;
