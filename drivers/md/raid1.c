...
static void make_request(struct mddev *mddev, struct bio * bio)
{
	...
	if (rw == READ) {
		 ...
		 */
		int rdisk;
        printk("MikeT: %s %s %d, read balancing logic\n", __FILE__, __func__, __LINE__);
read_again:
		rdisk = read_balance(conf, r1_bio, &max_sectors);
        printk("MikeT: %s %s %d, rdisk %d\n", __FILE__, __func__, __LINE__, rdisk);
		if (rdisk < 0) {
			    ...
			    int sectors, struct page *page, int rw)
{
	if (sync_page_io(rdev, sector, sectors << 9, page, rw, false))
		/* success */
		return 1;
	///MikeT skip here, don't update superblock
	
	if (rw == WRITE) {
		printk("MikeT: %s %s %d, can't write\n", __FILE__, __func__, __LINE__);
			   ...
			   sector_t sect, int sectors)
{
	...
	while(sectors) {
		...
		do {
			...
			rdev = conf->mirrors[d].rdev;
			printk("MikeT: %s %s %d, rdev: %p, In_sync: %d, Faulty: %d\n",
					__FILE__, __func__, __LINE__,
					rdev, test_bit(In_sync, &rdev->flags)?1:0, 
					test_bit(Faulty, &rdev->flags)?1:0);
			if (rdev &&
